...
After generating or editing slides, use the visual validator to render slides as PNG images and inspect them.

### How It Works

1. Run the validator script to render slides to PNG
2. Read the PNG images to visually inspect each slide
3. Check for spacing, alignment, overlap, and styling issues
4. Fix any issues found in the PPTX, then re-render to verify

### Running the Validator

```bash
# Render ALL slides
cd .claude/skills/pptx-generator
uv run --with aspose-slides python validate-slides.py /path/to/slides.pptx

# Render specific slides only
uv run --with aspose-slides python validate-slides.py /path/to/slides.pptx --slides 1,5,16,38

# Custom output directory
uv run --with aspose-slides python validate-slides.py /path/to/slides.pptx --output-dir ./my-preview
```

Output goes to `.tmp/preview/` by default (slide_01.png, slide_02.png, etc.).

### Visual Inspection Checklist

After rendering, read each PNG and check for:

- [ ] **Spacing**: Even gaps between elements, no cramped areas
- [ ] **Alignment**: Elements that should be aligned ARE aligned (titles, cards, bullets)
- [ ] **Overlap**: No unintended shape overlap (except intentional layering like card backgrounds)
- [ ] **Empty space**: Content should fill the slide appropriately (not just top half)
- [ ] **Text fit**: Text doesn't overflow its container or get truncated
- [ ] **Consistency**: New slides match the visual style of existing slides
- [ ] **Background**: Correct background color on all slides
- [ ] **Typography**: Font sizes and weights match the presentation's style

### When to Validate

- **Always** after generating new slides
- **Always** after combining batches into a final PPTX
- **Always** after programmatic edits (text replacement, reordering)
- Before delivering the final PPTX to the user

### Note on Aspose Watermark

The free version of aspose-slides adds an "Evaluation only" watermark to rendered images. This does NOT affect the actual PPTX file - it only appears in the preview PNGs. Ignore the watermark when inspecting slides.
